...
def sql(funct):
    ...
    def wrap_function(*args, **kwargs):
        ...
        with psycopg.connect(user='employees', password='employees', host=host, dbname='employees') as conn:
            kwargs['conn'] = conn
...
def save(command, conn = None):
    with conn.cursor() as cursor:
        cursor.execute("insert into employees(emp_name) values (%s) returning id", (command["name"],))
        conn.commit()
